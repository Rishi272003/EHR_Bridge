...
from services.ehr.practice_fusion.categories.DocumentReference import DocumentReference


class DocumentReferenceQueryTransformer(Transformer):
    """
    Transformer for Practice Fusion DocumentReference queries.
    Transforms FHIR DocumentReference responses to standardized format.
    ...
    def transform(self):
        """
        Transform document reference query request and fetch data from Practice Fusion.
        """
        try:
            ...
            # Build search parameters and fetch data
            search_params = {}
            if document_id:
            ...
            if date_params:
                search_params["date_params"] = date_params
            if encounter_id:
    ...
    def _transform_fhir_response(self, fhir_response):
        ...
        Handles multiple DocumentReference resources.
        """
        documents = []
        ...
        if fhir_response.get("resourceType") == "Bundle":
            ...
            # Process all DocumentReference resources
    ...
    def _transform_document_reference_resource(self, resource, full_url=None):
        """
        ...
        # Add master identifier if present
        master_identifier = resource.get("masterIdentifier", {})
        if master_identifier.get("value"):
            doc_obj["Identifiers"].append({
                "ID": master_identifier.get("value"),
                "IDType": "MasterIdentifier",
                "System": master_identifier.get("system")
            })
            ...
            if isinstance(encounters, list):
                for enc in encounters:
                    doc_obj["Context"]["Encounter"].append({
                        "Reference": enc.get("reference") if isinstance(enc, dict) else enc,
                        "Display": enc.get("display") if isinstance(enc, dict) else None,
                    })
            elif encounters:
                # Single encounter reference
                if isinstance(encounters, dict):
                    doc_obj["Context"]["Encounter"].append({
                        "Reference": encounters.get("reference"),
                        "Display": encounters.get("display"),
                    })
                else:
                    doc_obj["Context"]["Encounter"].append({
